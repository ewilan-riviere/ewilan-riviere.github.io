<template>
  <div>
    <div class="mb-10">
        <svg
          class="w-auto h-10 mx-auto md:h-20"
          viewBox="0 0 273 64"
          fill="none"
        ...
        <div class="flex mx-auto mt-5 w-max">
          <svg
            class="w-auto h-20"
            viewBox="0 0 257 169"
            fill="none"
          ...
          >
            <path
              d="M81.1543 128.231H116.995V136.499H81.1543V143.314H193.876V136.499H152.088V128.231H198.565V109.353H81.1543V128.231Z"
              fill="black"
            />
            <path
              d="M49.9403 14.6415H220.818V82.944H235.459V0H35.2988V45.7402H49.9403V14.6415Z"
              fill="black"
            />
            <path
              d="M0 164.993H76.4337V50.3345H0V164.993ZM39.8299 159.235C37.7985 159.235 36.1459 157.593 36.1459 155.556C36.1459 153.525 37.7985 151.878 39.8299 151.878C41.8667 151.878 43.5087 153.52 43.5087 155.556C43.5087 157.593 41.8667 159.235 39.8299 159.235ZM11.0522 61.3867H65.3921L65.3868 145.973H11.0522V61.3867Z"
              fill="black"
            />
            <path
              d="M203.754 87.3496V168.067H256.999V87.3496H203.754ZM231.458 90.6863C232.095 90.6863 232.616 91.2179 232.616 91.8494C232.616 92.4862 232.095 93.0178 231.458 93.0178C230.811 93.0178 230.284 92.4862 230.284 91.8494C230.284 91.2126 230.805 90.6863 231.458 90.6863ZM224.543 94.5598H238.347V95.6756H224.543V94.5598ZM230.995 164.583C229.463 164.583 228.237 163.341 228.237 161.82C228.237 160.294 229.463 159.062 230.995 159.062C232.516 159.062 233.758 160.294 233.758 161.82C233.758 163.336 232.516 164.583 230.995 164.583ZM249.299 155.078C236.837 155.147 217.638 155.194 211.464 155.142V99.6438L249.299 99.6386V155.078Z"
              fill="black"
            />
          </svg>
        </div>
        <div
          class="font-sans text-xl font-semibold text-center text-black md:text-3xl"
        >
          vue-tailwind-screens-helper (VTSH)
        </div>
      </div>

        <div class="flex pl-0 lg:pl-5" v-for="item in list" :key="item.id">
          <span class="block mr-2">-</span>
          {{ item }}
        </div>
        <div class="pl-0 mt-5 lg:pl-5">
          <div class="text-xl font-semibold">Options</div>
          <div class="flex mt-5">
            <button
              class="inline-block px-6 py-2 mr-3 text-xs font-semibold leading-6 text-center text-white uppercase transition bg-black rounded shadow ripple waves-light hover:shadow-lg focus:outline-none hover:bg-black"
              @click="isSneak = !isSneak"
            >
              Toggle sneak mode
            </button>
            <button
              class="inline-block px-6 py-2 mr-3 text-xs font-semibold leading-6 text-center text-white uppercase transition bg-black rounded shadow ripple waves-light hover:shadow-lg focus:outline-none hover:bg-black"
              @click="ixPixels = !ixPixels"
            >
              Toggle pixels width
            </button>
          </div>
      </div>
    <tailwind-helper :sneak="isSneak" :pixels="ixPixels" force-display></tailwind-helper>
  </div>
</template>
